...
class ThemeModeHelper {
  static ThemeMode getThemeMode(dynamic input, {Map<ThemeModeOption, ThemeMode>? customThemeModes}) {
    final option = ThemeModeConverter.fromDynamic(input);
    return customThemeModes?[option] ?? ThemeModeConstants.defaultData[option]!.themeMode;
  }

  static IconData getThemeModeIcon(dynamic input, {Map<ThemeModeOption, IconData>? customIcons}) {
    final option = ThemeModeConverter.fromDynamic(input);
    return customIcons?[option] ?? ThemeModeConstants.defaultData[option]!.icon;
  }

  static String getThemeModeLabel(dynamic input, {Map<ThemeModeOption, String>? customLabels}) {
    final option = ThemeModeConverter.fromDynamic(input);
    return customLabels?[option] ?? ThemeModeConstants.defaultData[option]!.label;
  }
}
